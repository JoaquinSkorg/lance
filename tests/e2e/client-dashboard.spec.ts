...
test.describe("Client Dashboard", () => {
  test.beforeEach(async ({ page }) => {
    ...
    await expect(page.locator("h1")).toContainText(
      "Manage hiring and escrow milestones",
    );
  });

  test("should display client metrics and active registry", async ({
    page,
  }) => {
    await expect(page.locator("h1")).toContainText(
      "Manage hiring and escrow milestones",
    ...
    await expect(
      page.getByRole("heading", { name: "Escrow Volume" }),
    ).toBeVisible();

    // Check active registry
    await expect(
      page.getByRole("heading", { name: "Active Registry" }),
    ).toBeVisible();
    await expect(
      page.locator('div[class*="group flex items-center justify-between"]'),
    ).toHaveCount(5);
  });
});
