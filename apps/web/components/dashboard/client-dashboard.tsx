    ...
    <div className="space-y-8 animate-in fade-in duration-700">
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
        {stats.map((stat) => (
          <Card
            key={stat.label}
            className="border-border/60 bg-background/50 backdrop-blur-sm overflow-hidden group hover:border-primary/30 transition-all duration-300"
          >
            ...
            <CardContent>
              ...
              <p className="text-[10px] text-muted-foreground mt-1 flex items-center gap-1">
                <span className="text-emerald-500 font-bold">+12%</span> from
                last cycle
              </p>
            </CardContent>
      ...
      <div className="grid gap-6 lg:grid-cols-[1fr_0.45fr]">
        <Card className="border-border/60 bg-zinc-950 text-zinc-50 dark:bg-card dark:text-card-foreground">
          <CardHeader className="flex flex-row items-center justify-between">
            <div className="space-y-1">
              ...
              <CardDescription className="text-zinc-400">
                Monitor your open briefs and milestone posture.
              </CardDescription>
            </div>
            <Link
              href="/jobs/new"
              className="inline-flex items-center gap-2 rounded-full bg-primary px-4 py-2 text-sm font-semibold text-primary-foreground hover:bg-primary/90 transition-all shadow-lg shadow-primary/20"
            >
          ...
          <CardContent>
            <div className="space-y-4">
              {loading
                ...
                : activeJobs.map((job) => (
                    <div
                      key={job.id}
                      className="group flex items-center justify-between rounded-2xl border border-white/5 bg-white/[0.02] p-4 hover:bg-white/[0.05] transition-all duration-200"
                    >
                      ...
                      <Link
                        href={`/jobs/${job.id}`}
                        className="p-2 rounded-full hover:bg-white/10 transition-colors text-zinc-500 hover:text-zinc-100"
                      >
            ...
            <Button
              variant="ghost"
              className="w-full mt-6 rounded-xl border border-white/5 text-zinc-400 hover:text-zinc-100 hover:bg-white/5"
            >
          ...
          <Card className="border-primary/20 bg-primary/5 relative overflow-hidden">
            ...
            <CardContent>
              <p className="text-xs text-muted-foreground leading-relaxed">
                Your account is currently aligned with all on-chain reputation
                requirements.
              </p>
            </CardContent>
          </Card>
