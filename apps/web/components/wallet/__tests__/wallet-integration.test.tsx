...
describe("Wallet Integration", () => {
  ...
  describe("ConnectWalletButton", () => {
    ...
    it("displays connected state with wallet address", () => {
      mockUseWalletSession.mockReturnValue({
        address:
          "GD5T2P2Q7F3X4Y6Z8R1W3E5T6Y7U8I9O0P1Q2R3S4T5U6V7W8X9Y0Z1A2B3C4D5",
      ...
      expect(
        screen.getByRole("button", { name: /disconnect wallet gd5t…c4d5/i }),
      ).toBeInTheDocument();
    });
    ...
    it("shows network mismatch warning", () => {
      ...
      expect(
        screen.getByText("Wallet: PUBLIC | App: TESTNET"),
      ).toBeInTheDocument();
      expect(
        screen.getByText("Please switch your wallet network to match"),
      ).toBeInTheDocument();
    });
  ...
  describe("WalletErrorDisplay", () => {
    it("displays user rejection error with recovery steps", () => {
      ...
      expect(
        screen.getByText("Check your wallet extension popup"),
      ).toBeInTheDocument();
      expect(
        screen.getByText("Click 'Approve' or 'Connect' in your wallet"),
      ).toBeInTheDocument();
    ...
    it("displays wallet not found error with installation steps", () => {
      const error =
        "Wallet extension not found. Please install a supported wallet (Freighter, Albedo, or xBull).";

      render(<WalletErrorDisplay error={error} />);

      expect(screen.getByText(error)).toBeInTheDocument();
      expect(
        screen.getByText("Install Freighter, Albedo, or xBull wallet"),
      ).toBeInTheDocument();
      expect(
        screen.getByText("Enable the wallet extension in your browser"),
      ).toBeInTheDocument();
      expect(
  ...
  describe("WalletConnectionModal", () => {
    it("renders wallet selection modal", () => {
      ...
      expect(
        screen.getByText("Choose your Stellar wallet"),
      ).toBeInTheDocument();
    ...
    it("handles wallet selection", async () => {
      ...
      const freighterButton = screen.getByRole("button", {
  ...
  describe("Accessibility Compliance", () => {
    ...
    it("announces loading states to screen readers", () => {
      ...
      expect(screen.getByText("Securing connection")).toHaveAttribute(
        "aria-live",
        "polite",
      );
    });
    ...
    it("announces errors to screen readers", () => {
      ...
      expect(screen.getByText("Connection failed")).toHaveAttribute(
        "aria-live",
        "assertive",
      );
    });
