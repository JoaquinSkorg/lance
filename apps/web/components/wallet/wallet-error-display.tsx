    ...
    >
      <div className="flex items-start gap-3">
        <div className="flex-shrink-0">
          <AlertTriangle
            className={cn("h-5 w-5 animate-pulse", colors.icon)}
            aria-hidden="true"
          />
        </div>
        ...
        <div className="flex-1 min-w-0">
          ...
          <p className="text-xs text-zinc-300 mb-3 leading-relaxed">{error}</p>
...
export function WalletErrorBanner({
  ...
  return (
    ...
    >
      <AlertTriangle
        className="h-4 w-4 text-red-400 animate-pulse flex-shrink-0"
        aria-hidden="true"
      />
      <div className="flex-1 min-w-0">
        <p className="text-xs text-red-400 font-medium truncate">{error}</p>
      </div>
      {onRetry && (
        <Button
          variant="ghost"
